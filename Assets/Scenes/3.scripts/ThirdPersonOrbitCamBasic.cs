...
public class ThirdPersonOrbitCamBasic : MonoBehaviour 
{
	...
	public float rotationSpeed = 5f; // Speed at which the camera rotates towards the player
	public bool FOVmode=true;
	...
	void Update()
	{
		...
		// Mouse:
		angleH += Mathf.Clamp(TCKInput.GetAxis("Touchpad",EAxisType.Horizontal), -1, 1) * horizontalAimingSpeed;
		angleV += Mathf.Clamp(TCKInput.GetAxis("Touchpad",EAxisType.Vertical), -1, 1) * verticalAimingSpeed;
		// Joystick:
		...
		//angleV += Mathf.Clamp(Input.GetAxis(YAxis), -1, 1) * 60 * verticalAimingSpeed * Time.deltaTime;
            if( TCKInput.GetAction( "fireBtn", EActionEvent.Press) || FOVmode)
            {
                SetFOV();
            }
			else
	    ...
	    void SmoothLookAtPlayer()
    {
        ...
        // Get the target rotation to look at the player
